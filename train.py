...
if __name__=='__main__':
    ...
    model = HydraSalsa(nclasses=[20,23], inchannels=5)
    ...
    for epoch in range(max_epochs):
        ...
        if mean_iou_kitti>best_mean_iou_kitti:
            ...
            torch.save(model.state_dict(), 'best_kitti_state_dict.pth')
        ...
        if mean_iou_waymo>best_mean_iou_waymo:
            ...
            torch.save(model.state_dict(), 'best_waymo_state_dict.pth')
            print("waymo model is saved")

    torch.save(model.cpu().state_dict(), 'last_model_state_dict.pth')
